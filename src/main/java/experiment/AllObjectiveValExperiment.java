...
public class AllObjectiveValExperiment
{
    ...
    private static ProblemInstance getProblemInstance(int size)
    {
        ...
        try
        {
            switch (size)
            {
                case 17 -> pI = DataReader.readFileForGraphMatrix(filepath + "br17.atsp");
                case 33 -> pI = DataReader.readFileForGraphMatrix(filepath + "ftv33.atsp");
                case 55 -> pI = DataReader.readFileForGraphMatrix(filepath + "ftv55.atsp");
                case 70 -> pI = DataReader.readFileForGraphMatrix(filepath + "ftv70.atsp");
                case 124 -> pI = DataReader.readFileForGraphMatrix(filepath + "kro124p.atsp");
                case 403 -> pI = DataReader.readFileForGraphMatrix(filepath + "rbg403.atsp");
                default -> {
