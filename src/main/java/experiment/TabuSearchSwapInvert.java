...
public class TabuSearchSwapInvert
{
    public static void generateDataTests()
    {
        System.out.println("Swap vs invert efficiency");
        try
        {
            String filePath = System.getProperty("user.dir") + "/results/";

            String filename = filePath + "TabuSearchSwapInvert" + ".txt";
            PrintWriter writer = new PrintWriter(filename, StandardCharsets.UTF_8);
            
            writer.println("TABU SWAP INVERT COMP");
            ...
            int ageLimit = 50;
            writer.println("ITERATIONS invert swap");
            for (int iterations = 100; iterations <= 3000; iterations += 200)
            {
                System.out.println("iterations " + iterations);
                writer.print(iterations + " ");
                
                TabuSolver[] solvers = {new TabuSolver(ageLimit, iterations, false, false, false), new TabuSolver(ageLimit, iterations, false, false, true)};
